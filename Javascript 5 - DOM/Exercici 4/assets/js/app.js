window.addEventListener("load", () => {
    ...
    const removeCartItem = (event) => {
        ...
        let numItemAdded = document.querySelector("tbody").childElementCount
        if (numItemAdded === 0) {
            toggleShoppingControls(false)
        }
        updateCartTotal()
    ...
    // Add item to cart afeter clickling cart button
    ...
    const addItemToCart = (productId, title, productName, price, imageSrc) => {
        ...
        cartRow.lastElementChild.firstElementChild.firstElementChild.firstElementChild.addEventListener("input", () => {
            updateCartTotal();
        })
