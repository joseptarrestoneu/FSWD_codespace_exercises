const GenerateContent = class {
    ...
    getProductFormat = () => {
        let product = `
            <div class="draggable card mb-4 shadow-sm" draggable="true" product-id="xxx">
            <img
                src="./img/gallery/fulls/fr-nnn.jpg"
                class="card-img-top"
                alt="fruitName"
                draggable="false"
            />
    ...
    setPageContent = (fruits) => {
        ...
        for (const key in fruits) {
            ...
            card = card.replace(/xxx/,fruits[key][0])
            card = card.replace(/nnn/,fruits[key][0])
            card = card.replace(/fruitName/,fruits[key][1])
            card = card.replace(/yyy/,fruits[key][3])
            card = card.replace(/zzz/,fruits[key][1].trim())
